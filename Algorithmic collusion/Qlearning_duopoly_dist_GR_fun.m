function output=Qlearning_duopoly_dist_GR_fun()
...
Q_0(:,:,:,1,1) = flip(Q_0(:,:,:,1,2),3);
Q_0(:,:,:,2,1) = flip(Q_0(:,:,:,2,2),3);

% Random initial Q-matrix
%Q_0 = unifrnd(min(Q_0,[],'all'),max(Q_0,[],'all'),n_prices, n_prices, n_prices, n_players, n_init);
...
for t = 2:n_periods
    ...
    for s = 1:n_prices
        Q_vec(s,1,1) = Q(state1(1),:,s,1,1) * (joint_dist(state1(1),:,1)/sum(joint_dist(state1(1),:,1)))';
        Q_vec(s,2,1) = Q(:,state1(2),s,2,1)' * (joint_dist(:,state1(2),1)/sum(joint_dist(:,state1(2),1)));

        Q_vec(s,1,2) = Q(state2(1),:,s,1,2) * (joint_dist(state2(1),:,2)/sum(joint_dist(state2(1),:,2)))';
        Q_vec(s,2,2) = Q(:,state2(2),s,2,2)' * (joint_dist(:,state2(2),2)/sum(joint_dist(:,state2(2),2)));
    end
    ...
    for i = 1:n_players
        ...
        if rand < epsilon
            ...
            exploration_range1 = price_range(max(1,state1(i)-kbar):min(15,state1(i)+kbar));
            prices(i,t,1) = exploration_range1(randi(length(exploration_range1)));
        else          
    ...
    for i = 1:n_players
        ...
        if rand < epsilon
            ...
            % Choose a random price from the previous price state
            exploration_range2 = price_range(max(1,state2(i)-kbar):min(15,state2(i)+kbar));
            prices(i,t,2) = exploration_range2(randi(length(exploration_range2)));
        else
            % Choose the best price based on Q-vector
            [~,idx2] = max(Q_vec(:,i,2));
    ...
    action1 = [find(price_range == prices(1,t,1)),find(price_range == prices(2,t,1))];
    action2 = [find(price_range == prices(1,t,2)),find(price_range == prices(2,t,2))];
    ...
    for i = 1:n_players
        Q(state1(1),state1(2), action1(i), i,1) = (1-alpha) * Q(state1(1),state1(2), action1(i), i,1) + ...
            alpha * (payoff(i,t,1) + delta * max(Q(action1(1),action1(2), :, i,1)));

        Q(state2(1),state2(2), action2(i), i,2) = (1-alpha) * Q(state2(1),state2(2), action2(i), i,2) + ...
            alpha * (payoff(i,t,2) + delta * max(Q(action2(1),action2(2), :, i,2)));
    end
    ...
    if mod(t,1000000)==0
        ...
        W(:,:) = (1/(2*(n-1)))*((prices(:,t-999999:t,1)-avg_1)*(prices(:,t-999999:t,1)-avg_1)' + (prices(:,t-999999:t,2)-avg_2)*(prices(:,t-999999:t,2)-avg_2)');
