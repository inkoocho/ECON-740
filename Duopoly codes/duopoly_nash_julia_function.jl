    ##
##    Written by Jung Jae Kim on January 7, 2024. Translated from duopoly_nash_fun.m file
##

function duopoly_nash_julia_function(N,lambda,sigmawn)
    #N = 10000;
    ...
    RR1 = Matrix{Float64}(I, 3, 3);
    RR2 = Matrix{Float64}(I, 3, 3);
    ...
    for t = 1:N-1
        ...
        beta1[:,t+1] = beta1[:,t] + lambda*inv(RR1)*[1; p[1,t]; p[2,t]]*(q1 -beta1[1,t]+beta1[2,t]*p[1,t]-beta1[3,t]*p[2,t]);
        RR1 = RR1 + lambda*( [ 1; p[1,t]; p[2,t]]*[1 p[1,t] p[2,t]] - RR1);
        beta1[:,t+1] = max.([0;0;0],beta1[:,t+1]);
        ...
        beta2[:,t+1] = beta2[:,t] + lambda*inv(RR2)*[1; p[2,t]; p[1,t]]*(q2 -beta2[1,t]+beta2[2,t]*p[2,t]-beta2[3,t]*p[1,t]);
        RR2 = RR2 + lambda*( [ 1; p[2,t]; p[1,t]]*[1 p[2,t] p[1,t]] - RR2);
        beta2[:,t+1] = max.([0;0;0],beta2[:,t+1]);

    end
    
    return vcat(br, profit); 
end