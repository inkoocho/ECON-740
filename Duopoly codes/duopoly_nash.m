...
% [p_nash1,p_nash2]
% [p_collude1,p_collude2]
% (p_collude2 - p_nash2)/(p_collude1-p_nash1)
%
% %threshold of alpha for triggering escape
%
% alphaunderbar1=sigmawn*sigmawn*( 1+2*C2*q_nash1/p_nash2)/(4*C1*C2*q_nash1*q_nash2);
% alphaunderbar2=sigmawn*sigmawn*( 1+2*C1*q_nash2/p_nash1)/(4*C1*C2*q_nash1*q_nash2);
...
%hold on;
%%subplot(1,3,2)
%plot(p_nash1,p_nash2,'b*')
%plot(p_collude1,p_collude2,'r*')
%plot(pavg(1,1:N-1),pavg(2,1:N-1))
%hold off

%figure;
%hold on;
%subplot(1,3,3)
%xline(alphaunderbar1, '--r')
%yline(alphaunderbar2,'--.r')
%plot(alpha1(2,1:N-1),alpha2(2,1:N-1));
%hold off
...
plot(br(1,1:N-1),br(2,1:N-1))
plot(p_nash1,p_nash2,'b*')
plot(p_follower,p_leader,'r*')
plot(p_collude1,p_collude2,'g*')
ylim([p_nash1-0.5 p_collude1+0.1])
lgd=legend('Path','Nash ($\frac{A}{2B-C}$)','Stackelberg','Collude ($\frac{A}{2(B-C)}$)','Location','northwest','Interpreter','latex');
lgd.FontSize = 10;
hold off
...
plot(br(2,1:N-1))
ylim([p_nash1-0.5 p_collude1+0.1])
lgd=legend('Firm 1 BR','Firm 2 BR','Location','northwest');
lgd.FontSize = 10;
hold off
...
plot(profit(1,1:N-1))
plot(profit(2,1:N-1))
hline = refline([0 p_collude1*q_collude1]);
hline.Color = 'g';
ylim([0 1.5])
lgd=legend('Firm 1 payoff','Firm 2 payoff','Collusion payoff','Location','northwest');
lgd.FontSize = 10;
hold off

 [1/N*sum(profit(1,:)) 1/N*sum(profit(2,:))]