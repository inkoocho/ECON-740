...
function gresham_kernel!(price, pi1, beta0, beta1, Nsim, Titer, alpha, rho, sigmap, sigmaf, sigmab, delta, sce)
    ...
    for t = 2:Titer
        ...
        MSE1 = sigmap + V1 * f^2 + f^2 * V1 * (1 - alpha * pi_i)^2
        MSE0 = sigmap + (alpha * pi_i)^2 * V1 * f^2 + f^2 * V0 * (1 - alpha * (1 - pi_i))^2 + V0 * f^2
        ...
        r += log(A1 / A0)
        pi_i = 1 / (1 + exp(-r))
        f = rho * f + vf # moved  - to interpret f as f(-2) and vf as vf(-1)
        price_i = (delta + alpha * (pi_i * beta1_i + (1 - pi_i) * beta0_i)) * f + vp - alpha * (pi_i * beta1_i + (1 - pi_i) * beta0_i) * vf
