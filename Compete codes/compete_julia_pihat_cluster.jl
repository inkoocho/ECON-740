...
results = @time @sync @distributed (vcat) for lll in xt
    ...
    for i=2:Titer             
        ...
        #res0 = price[i-1] - (pi1[i-1]*beta1[i-1] + (1-pi1[i-1])*(beta0[i-1] + sce*f[i-1]))
        ...
        pi1[i] = 1/(1 + exp(-r[i]))
        pihat1[i] = pihat1[i-1] + (1/(50+i))*(pi1[i] - pihat1[i-1])  
    ...
    compete_path = hcat(j, k, m, pi1[Titer], pihat1[Titer]);
end
...
result_arrange = zeros(Numf,Numb,Nsim,2)
for i=1:Numf*Numb*Nsim
    j=Integer(results[i,:][1])
    k=Integer(results[i,:][2])
    m=Integer(results[i,:][3])
    result_arrange[k,m,j,1]=results[i,:][4]
    result_arrange[k,m,j,2]=results[i,:][5]
end
...
#end
av_pi=zeros(Numf,Numb);
av_pihat=zeros(Numf,Numb);
for k=1:Numf, m=1:Numb
   av_pi[k,m]=sum(result_arrange[k,m,:,1])/length(result_arrange[k,m,:,1]);
   av_pihat[k,m]=sum(result_arrange[k,m,:,2])/length(result_arrange[k,m,:,2]);
end

#df = DataFrame(avg_Pi = vec(av),sigmaf = repeat(sigmaf,outer=Nump), sigmap = repeat(sigmap,inner=Numf), sigmab=sigmab )
df = DataFrame(avg_Pi = vec(av_pi), avg_Pihat = vec(av_pihat), sigmaf = repeat(sigmaf,outer=Numb), sigmab = repeat(sigmab,inner=Numf), sigmap=sigmap, alpha=alpha )
